...
export const PROTOTYPES: Prototype[] = [
  ...
  },
  {
    ...
    desc: "把 blog 变成配置化的一整套:运营在后台写完、配好、点发布,前台立刻多一篇文章页,不需要为每篇 blog 单独开发。对标 Sanity / Storyblok / Payload 那条 headless CMS 路线 —— 正文用 block 数组建模,前端一个 renderBlock 分发渲染,连 CTA 卡、引用、代码块这些「以前必须开发介入」的富组件也变成可配 block。入口在共享组件 site-header 的 Resources 下拉(原本是一张写着 Coming soon 的空卡,现已填成 Blog / Help Center / Affiliate),改一处,homepage、MCP、seedance-2-5 三个原型同时有了 Blog 入口。页面顶部常驻一条演示切换(「Blog 落地页」/「Blog 发布后台」),两边互跳。① 落地页 /prototypes/blog —— 深色影院底(与仓库里 seedance-2-5 同一张底色),首屏是整幅 Buzz 橙 banner「BuzzVideo Resources」,右半铺自绘的几何纹样(方格骨架 + 格内四枚内缩三角 + 交点菱形)并向左渐隐;下面是 16px 粗体胶囊分类条(选中反白填充、带条目计数)+ 搜索;正文区刻意用四种不同版式避免一排等宽卡片:一条橙色规则线起头的 5/7 非对称头条(标题最大 50px)、中缝竖线分隔的两栏次条、编号 + 细线的紧凑存档行;卡片图即文章的封面图,滚到底自动加载下一批。② 详情页 /prototypes/blog/[slug] —— 一套模板服务所有文章:分类标签 → 标题 → 摘要 → 日期起头(封面图只上卡片和分享预览,不进正文头部)、左侧 sticky 目录(由 heading block 自动生成,点击平滑滚动)、720px 阅读栏宽的区块化正文、标签行、同分类相关文章;未发布的文章从后台预览时顶部挂深色 Preview 状态条。③ Admin /prototypes/blog/admin —— 4 个 KPI(累计发布 / 本月发布 / 草稿 / 归档)、All / Published / Drafts / Archived 四个状态 Tab、分类与作者下拉、标题 slug 搜索、逐行 Edit / Publish / Unpublish / Duplicate / Archive / Delete(破坏性操作走确认弹窗);另有 Blog Categories 一页管分类。新建文章三条路:粘贴 Google Docs、上传 .docx、从空白开始 —— 前两条走 mammoth 解析成 block,导入前弹审阅窗,列出解析出的块数与类型分布,并明说哪些东西没带过来(无 Title 样式、Callout/CTA 在 Docs 无对应物、图片已压缩或跳过);导入是单向的,导完不再与 Docs 同步。编辑器左侧常驻一条区块工具栏,十二种类型(段落 / 标题 / 图 / 列表 / 引用 / Callout / 视频 / CTA 卡 / 分隔线 / 表格 / 参考资料 / 代码)按使用频率排,点一下插到当前选中块之后、没选中则追加到末尾,块与块之间的缝隙 hover 出 + Insert 精确插入;每块带序号与类型标签、选中态用品牌橙描边、可上下移 / 复制 / 删除,按住六点手柄可拖拽重排。图片与封面图支持点选和拖拽上传,canvas 在浏览器端压到长边 1600px;视频块可贴 YouTube / Vimeo 链接(转 nocookie embed)或直传文件(存 IndexedDB,localStorage 放不下)。Write / Preview 双视图即时切换,Preview 的版式与前台文章页逐行对齐。右侧配置面板三段折叠且可独立滚动 —— Publish(状态与发布日只读,发布时间就是点 Publish 的那一刻)、Content(封面图、slug、分类、作者、标签)、SEO(meta title/description 带字数计,外加仿 Google 搜索结果与社交分享卡片两个实时预览,卡片图即封面图)。保存与发布都带按钮 loading 和统一的顶部 toast(成功绿 / 失败红)。后台与前台读同一个 store(localStorage 持久化 + useSyncExternalStore 订阅),所以在后台改标题、加一个 block、点发布,前台立刻跟着变 —— 这就是「配置即上新」要在评审里演示的闭环。演示数据可在控制台敲 __resetBlogDemo() 还原。纯前端 mock,不落库;SEO 标签由客户端写入,真实实现需改服务端渲染,见 docs/superpowers/specs/2026-09-15-blog-seo-交接.md。",
    date: "2026-09-14",
  ...
  },
  {
    ...
    desc: "整个团队 / 组织需求的索引 —— 把它摊成一张可逐条走查的清单,给开发、测试、评审共用一个口径。**左侧按用户旅程排,不按功能分类**:起点只有个人账户 → 建团队要去订阅、付款、带着一个已付费团队回来 → 把人拉进来 → 然后是团队设置的全部七个 tab(General / Members 与席位 / Permissions & roles / Credits & usage / Top-up / Billing / Activity log,与产品里设置面板的顺序一一对应)→ 配完之后的日常使用(撞墙与申请回路)→ 通知与邮件 → 最后是客户看不到的我们这一侧(sales 开户、内部用量管控)。共 13 个阶段、34 条功能,列表项带实现状态图标(已打通 / 部分实现 / 尚未实现)与缺口计数徽章,支持按功能名或边界状态文案搜索,以及「只看有缺口 / 待决策的」一键过滤。右侧是单条功能的评审面板:一句话讲清它解决谁的什么问题,下面是编号的「边界状态与规则」逐条清单(测试可以直接当用例走),再下面是「在原型里打开」——每个链接都通过 URL 参数把演示状态一次带进去(团队 / 角色 / 额度档位 / 席位 / 自动充值),比如「席位用尽 · Member」直接落到 per-seat 团队下 Member 视角额度耗尽的首页,「分配额度满 · Enterprise」落到共享池那套模型,不用再去底部演示条上手动切几个下拉;最后是琥珀色的「缺口 / 待决策」区块,诚实列出还没做或还没定的事(降档没有「待生效变更」状态所以无从取消、月付↔年付切换、付款失败的 dunning 链路、税号与账单地址、org-members 与 team-workspace 仍是两套数据等)。清单本身即需求文档,状态徽章不美化。",
    date: "2026-08-17",
