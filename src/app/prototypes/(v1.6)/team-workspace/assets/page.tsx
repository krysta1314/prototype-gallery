...
const BASE_ASSETS: Omit<Asset, "scope" | "ownerId" | "ownerName" | "ownerColor">[] = [
  {
    id: "a1",
    source: "ai",
    type: "image",
...
type Owner = { ownerId: string; ownerName: string; ownerColor: string };
const OWNER = (name: string, color: string): Owner => ({
  ownerId: name === ME.name ? ME.id : `u-${name.split(" ")[0]!.toLowerCase()}`,
  ownerName: name,
  ownerColor: color,
});

const withScope = (
  base: Omit<Asset, "scope" | "ownerId" | "ownerName" | "ownerColor">,
  idSuffix: string,
  scope: "private" | "team",
  owner: Owner,
): Asset => ({ ...base, id: `${base.id}${idSuffix}`, scope, ...owner });

const PRIVATE_BUCKET = "private";

const teamClone = (i: number, owner: Owner, suffix: string): Asset => {
  const base = BASE_ASSETS[i % BASE_ASSETS.length]!;
  return { ...withScope(base, suffix, "team", owner), title: `${base.title} (team)` };
};
...
const ASSET_BUCKETS: Record<string, Asset[]> = {
  ...
  "t-beauty": [
    teamClone(1, OWNER("Sofia Ruiz", "#e0568a"), "-bt0"),
    teamClone(2, OWNER("Mei Wong", "#f0a020"), "-bt1"),
  ],
};

/** Agent 视图(旧页面遗留)引用的扁平列表 */
const RAW_ASSETS: Asset[] = ASSET_BUCKETS[PRIVATE_BUCKET]!;
...
<div className="flex items-center gap-2.5"><TeamQuota /></div>
    </header>
  );
}
...
function AgentView() {
  // 额度用尽时发送按钮要换态并且点得动 —— 与 Home / Agent / Canvas 同一套状态
  const { role, quotaState, openSettings, openRequestModal, showToast } = useTeam();
          ...
          <div className="flex items-center justify-between gap-2 px-1 pt-2">
            ...
            {quotaBlocked ? (
              <button
                type="button"
                onClick={runQuotaCta}
                className="h-9 shrink-0 rounded-xl bg-[#c9432a] px-4 text-[12.5px] font-bold text-white transition hover:bg-[#ad361f]"
              >
                {quotaState.cta?.label ?? "Top up to continue"}
              </button>
            ) : (
              <button
                type="button"
                aria-label="Send"
                onClick={() => showToast("Generation isn't wired up in this prototype.")}
...
function AssetLibraryView() {
  ...
  /** 在私有桶与当前团队桶之间搬记录,同时改 scope */
  const moveAssets = (items: Asset[], from: string, to: string, scope: "private" | "team") => {
    const ids = new Set(items.map((a) => a.id));
    setBuckets((prev) => ({
      ...prev,
      [from]: (prev[from] ?? []).filter((a) => !ids.has(a.id)),
      [to]: [...items.map((a) => ({ ...a, scope })), ...(prev[to] ?? [])],
    }));
  };
  ...
  const visible = useMemo(() => {
    if (demoEmpty) return [];
    let list = assets.filter((a) => (isPersonal ? a.scope === "private" : a.scope === scope));
    ...
    return list;
  }, [assets, filter, source, favIds, sortOrder, query, demoEmpty, scope, isPersonal]);
  ...
  useEffect(() => {
    if (isPersonal && scope === "team") setScope("private");
  }, [isPersonal, scope]);

  const publish = (items: Asset[]) => {
    moveAssets(items, PRIVATE_BUCKET, team.id, "team");
    flash(items.length > 1 ? `Moved ${items.length} assets to Team Assets` : "Moved to Team Assets");
    setPendingPublish(null);
    setOpenId(null);
    exitSelect();
  };

  const unpublish = (item: Asset) => {
    moveAssets([item], team.id, PRIVATE_BUCKET, "private");
    flash("Moved to My Assets");
  };
  ...
  return (
    <div className="px-6 pb-24 pt-6">
      ...
      {/* scope 双区:My Assets / Team Assets */}
      {!isPersonal && (
        <div className="mb-5 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-1 rounded-xl border border-[#ececf1] bg-white p-1">
            {([
              { key: "private" as const, label: "My Assets", Icon: Lock },
              { key: "team" as const, label: "Team Assets", Icon: Users },
            ]).map(({ key, label, Icon }) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  aria-pressed={active}
                  onClick={() => { setScope(key); exitSelect(); }}
      ...
      {selecting && selected.size > 0 && (
        <div className="fixed bottom-4 left-1/2 z-40 flex max-w-[calc(100vw-1.5rem)] -translate-x-1/2 items-center gap-1 rounded-2xl bg-[#1a1a2e] px-2 py-2.5 text-white shadow-[0_16px_36px_rgba(26,26,46,0.28)] sm:bottom-6 sm:gap-2 sm:px-3">
          ...
          {!isPersonal && scope === "private" && (
            <button
              onClick={() => setPendingPublish(visible.filter((a) => selected.has(a.id)))}
              className="flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 text-sm font-semibold transition hover:bg-white/10 sm:px-3"
            >
              <Users className="size-4" /> <span className="hidden sm:inline">Publish to team</span>
            </button>
          )}
      ...
      {pendingPublish && pendingPublish.length > 0 && (
        <div className="fixed inset-x-0 bottom-0 top-[52px] z-[90] grid place-items-center bg-[#1a1a2e]/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Publish to team">
          <div className="w-full max-w-[420px] rounded-[24px] border border-[#ececf1] bg-white p-6 shadow-[0_30px_80px_rgba(26,26,46,0.28)]">
            <h2 className="text-[17px] font-bold tracking-[-0.02em] text-[#28222e]">
              {pendingPublish.length > 1
                ? `Publish ${pendingPublish.length} assets to the team?`
                : `Publish \u201c${pendingPublish[0]!.title}\u201d to the team?`}
            </h2>
            <p className="mt-2 text-[13px] leading-relaxed text-[#7b7480]">
              Everyone in {team.name} will be able to view and download{" "}
              {pendingPublish.length > 1 ? "them" : "it"}. Only you can edit or replace{" "}
...
function AssetCard({
  ...
  return (
    ...
    >
        ...
        <div className="absolute right-2 top-2 z-10 flex items-center gap-1">
          ...
          {onUnpublish && (
            <button
              ...
              aria-label="Move to my assets"
              title="Move to my assets"
              className="hidden size-7 place-items-center rounded-md bg-black/40 text-white backdrop-blur transition hover:bg-black/55 group-hover:grid"
            >
          ...
          <button
            ...
            disabled={!canDelete}
            aria-label="Delete"
            title={canDelete ? "Delete" : "Only the author, admins, or the owner can delete this."}
            className={`hidden size-7 place-items-center rounded-md bg-black/40 text-white backdrop-blur transition group-hover:grid ${
              canDelete ? "hover:bg-[#e05656]" : "cursor-not-allowed text-white/40"
            }`}
          >
      ...
      {!canEdit && !selecting && (
        <span className="pointer-events-none absolute left-2 top-2 z-10 flex items-center gap-1 rounded-md bg-black/45 px-1.5 py-1 text-[9px] font-bold uppercase tracking-wide text-white backdrop-blur">
