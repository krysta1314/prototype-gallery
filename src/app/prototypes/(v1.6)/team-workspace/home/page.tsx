          ...
          <div className="mt-5 grid items-stretch gap-4 min-[900px]:h-[clamp(270px,calc(468px-14.4vw),324px)] min-[900px]:grid-cols-[minmax(0,1.8fr)_minmax(0,1.6fr)] lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)] xl:grid-cols-[minmax(0,0.6fr)_minmax(0,1.1fr)] 2xl:grid-cols-[calc((100%_-_3rem)/3.5)_minmax(0,1fr)]">
            ...
            <div className="grid min-h-[280px] self-stretch grid-cols-2 gap-3 sm:min-h-[320px] min-[900px]:h-full min-[900px]:!min-h-0 min-[900px]:grid-cols-3 min-[900px]:grid-rows-2">
              {quickLinks.map(({ name, description, type, icon, badge }) => (
                <button
                  key={name}
                  onClick={() => (quotaBlocked ? onBlockedCard() : runQuickLinkAction(name))}
                  aria-disabled={quotaBlocked || undefined}
                  title={quotaBlocked ? quotaState.title : undefined}
                  className={`group relative flex min-h-[136px] min-w-0 flex-col overflow-hidden rounded-2xl p-4 text-left transition sm:min-h-[154px] sm:p-5 min-[900px]:!min-h-0 min-[900px]:!p-4 ${
                  quotaBlocked ? "cursor-not-allowed opacity-45 grayscale" : "hover:-translate-y-0.5"
                } ${
                  name === "Seedance 2.5"
...
const SHELL_NAV = [
  { key: "home", label: "Home", icon: ICONS.home, href: "/prototypes/team-workspace/home", active: true },
  { key: "agent", label: "Marketing Agent", icon: ICONS.marketing, href: "/prototypes/team-workspace/agent", active: false },
  { key: "canvas", label: "Canvas", icon: ICONS.canvas, href: "/prototypes/team-workspace/canvas", active: false },
] as const;
...
function HomepageWorkspace() {
  ...
  const promoUserState: PromoUserState = "member";
  const promoOfferPreviewState: PromoOfferPreviewState = "countdown";
  ...
  return (
    <div className="min-h-screen bg-white text-[#1a1a2e]" style={{ fontFamily: APPLE_FONT }}>
      ...
      <div className="flex">
        <aside className="sticky top-0 z-40 hidden h-screen w-[216px] shrink-0 self-start flex-col border-r border-[#ebe8ee] bg-white px-4 py-5 lg:flex">
          ...
          <nav className="mt-6 grid gap-1" aria-label="Primary navigation">
            ...
            <Link
              ...
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] font-semibold text-[#706a78] transition hover:bg-[#fff3ee] hover:text-[#ef6646]"
            >
        ...
        <main className="min-w-0 flex-1 pb-16 lg:pb-0">
          ...
          {/* 常驻额度横幅:80% / 100% 不能只靠顶栏胶囊变个色(评审第三节) */}
          <QuotaBanner className="mx-4 mb-2 sm:mx-6" />

          <HomepageContent embedded promoUserState={promoUserState} promoOfferPreviewState={promoOfferPreviewState} />
          <MarketingAgentPromptComposer
            scrollReactive
            quota={composerQuota}
            className="fixed bottom-5 left-1/2 z-50 w-[min(860px,calc(100vw-32px))] -translate-x-1/2 lg:left-[calc(50%+108px)]"
          />
        </main>
      </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex items-center gap-1 border-t border-[#ececf1] bg-white px-2 py-1.5 lg:hidden">
        ...
        <Link
          href="/prototypes/team-workspace/assets"
          className="flex flex-1 flex-col items-center gap-1 rounded-xl py-1.5 text-[11px] font-semibold text-[#706a78] transition"
        >
