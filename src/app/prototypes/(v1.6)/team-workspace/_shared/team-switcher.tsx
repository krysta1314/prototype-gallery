...
export function TeamSwitcher({ variant = "full" }: { variant?: "full" | "icon" }) {
  const { teamsOnly, personalTeam, hasTeams, team, memberCount, setActiveTeamId, openSettings, openAccount, setCreateTeamOpen } =
    useTeam();
    ...
    return (
      ...
      >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-0.5 flex items-center gap-1.5">
              ...
              <span className="truncate text-[11px] text-[#9a94a0]">
                {item.personal ? "· Just you" : `· ${people} ${people === 1 ? "member" : "members"}`}
              </span>
            </span>
          </span>
        </button>
        ...
        <button
          ...
          title={item.personal ? "Account settings" : "Team settings"}
    ...
    <div ref={rootRef} className={`relative ${variant === "icon" ? "" : "w-full"}`}>
      {variant === "icon" ? (
        <button
          ...
          aria-haspopup="menu"
          aria-expanded={open}
          aria-label={team.personal ? "Personal account. Switch workspace" : `Current team: ${team.name}. Switch team`}
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-0.5 flex items-center gap-1.5">
              ...
              {/* 个人账户没有成员数这个概念 */}
              <span className="truncate text-[11px] text-[#9a94a0]">
                {team.personal ? "· Just you" : `· ${activeMembers} ${activeMembers === 1 ? "Member" : "Members"}`}
              </span>
            </span>
          </span>
      ...
      {open && (
        ...
        >
          {/* 个人账户与团队分开列 —— 个人账户不是一个团队 */}
          <div onMouseLeave={() => setHoverId(null)}>
            <p className="px-2 pb-1 pt-1 text-[10px] font-bold uppercase tracking-[0.09em] text-[#a8a2ae]">
              Personal
            </p>
            <div className="grid gap-0.5">{renderRow(personalTeam)}</div>

            <p className="px-2 pb-1 pt-2.5 text-[10px] font-bold uppercase tracking-[0.09em] text-[#a8a2ae]">Teams</p>
            {hasTeams ? (
              <div className="grid gap-0.5">{teamsOnly.map(renderRow)}</div>
            ) : (
              // 注册后不自动送团队,所以这个空状态是新账户的常态
              <p className="px-2 pb-1 text-[12px] leading-[1.5] text-[#9a94a0]">
                No teams yet. Create one to invite teammates and share a credit pool.
              </p>
            )}
          </div>
          ...
          <div className="mt-1.5 border-t border-[#f0eef2] pt-1.5">
            <button
              ...
              className={`flex w-full items-center gap-2.5 rounded-xl px-2 py-2 text-[13px] font-semibold transition ${
                hasTeams ? "text-[#56505c] hover:bg-[#f6f4f7]" : "bg-[#fff0ea] text-[#ee6545] hover:bg-[#ffe7dc]"
              }`}
            >
              <Plus className={`size-4 ${hasTeams ? "text-[#8a8490]" : "text-[#ee6545]"}`} />
              Create team
            </button>
          </div>
        </div>
