            ...
            <div className="min-w-[240px] flex-1">
              ...
              <p className="mt-1 text-[12.5px] leading-snug text-[#8a8490]">
                {openUsage
                  ...
                  : "Only you, admins, and billing contacts can see per-teammate usage. Members see just their own."}
              </p>
            </div>
            <button
              type="button"
  ...
  const TYPES = [
    {
      ...
      label: "No limit",
      icon: InfinityIcon,
      help: "The member can generate freely until the team pool runs out.",
    },
    {
      key: "soft" as const,
      label: "Soft cap",
      icon: Bell,
      help: "Past the monthly limit the member gets a warning but can keep working while the team pool lasts.",
    },
    {
      key: "hard" as const,
      label: "Hard cap",
      icon: Lock,
      help: "Past the monthly limit the member can't submit any task, even if the team pool still has credits.",
    },
  ];
    ...
    <div className="fixed inset-x-0 bottom-0 top-[52px] z-[95] grid place-items-center bg-[#1a1a2e]/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Monthly limit">
      ...
      >
        <div className="relative text-center">
          {/* 「Balance」听起来像钱包余额,实际行为是每月上限 —— 统一叫 Monthly limit(评审第四节第 4 项) */}
          <h2 className="text-[17px] font-bold tracking-[-0.02em] text-[#28222e]">Monthly limit</h2>
...
function UsageCell({ member, onEdit }: { member: Member; onEdit: () => void }) {
  const { role, resendInvite, revokeInvite, canSeeTeammateUsage } = useTeam();
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-[11px] text-[#9a94a0]">
        {/* 「used this cycle」跟上面的上限对不上,改成明确的 X of Y this month */}
        {capped
          ? `${formatNumber(member.usedThisCycle)} of ${formatNumber(member.limit!.credits)} this month`
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-2 block text-[12px] font-semibold text-[#56505c]">
                    {option.seatsTotal} seats · {formatNumber(option.creditsTotal)} credits / month
                  </span>
                </span>
              </button>
            );
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-5 rounded-2xl border border-[#ececf1] bg-white p-5">
        ...
        />
        {/* AI Tokens 是内部口径,不对 Client 端暴露 —— 三条计量条并列却不解释彼此关系,
            只会让人搞不清到底哪个才是真正的限制(评审第 5 项) */}
...
function BillingReadOnly() {
  const { team, plan, nextBill, pool, seatsUsed, seatsTotal, openRequestModal, requests } = useTeam();
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl border border-[#ececf1] bg-white p-5">
        ...
        <dl className="mt-5 grid gap-4 border-t border-[#f0eef2] pt-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="mt-1 text-[15px] font-bold text-[#28222e]">{formatNumber(pool.remaining)}</dd>
          </div>
        </dl>
      </section>
      ...
      <section className="rounded-2xl border border-[#ececf1] bg-white p-5">
        ...
        <p className="mt-1 text-[12.5px] leading-snug text-[#8a8490]">
          Only the owner and billing contacts can pay, but you can ask them here — the request lands in their notifications, not just
          their inbox.
        </p>
        <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
          ...
          <button
            type="button"
            onClick={() => openRequestModal("credits")}
      ...
      <p className="text-[12px] leading-snug text-[#9a94a0]">
        Invoices and payment details stay with {team.name}&apos;s owner and billing contacts. Monthly limits reset on {nextBill}.
      </p>
    </div>
  );
}
...
function BillingTab() {
  ...
  // 现在改成只读展示 + 两个真的能发出去的申请(评审第四节第 2 项)。
