...
/**
 * 只有 Ultra 有滑杆（Starter / Pro 固定价）。三档:1× / 2× / 4×。
 *
 * 4× 曾在 rate card 评审里被删掉,2026-08-20 按 PM 决定复原。删它的理由留在这儿备查:
 * 4× 年付单价 $0.005/credit,比 Scale 席位便宜 29%,只想要产能的 agency 会买
 * 3 个 Ultra 4× 共用账号而不是 10 个 Scale 席位;减 40% 营销只剩 7% 毛利,
 * cost factor ×2 会转负。要挡这条路得靠 Business 档的能力差(共享池、权限、发票),
 * 不能只靠价格。
 *
 * 定价红线(参考值):任何 individual 档的 credit 单价不宜低于 Scale 席位单价的 90%。
 * Scale 年付是 $0.00704,所以下限是 $0.00634 —— 4× 年付低于这条线,是明知的例外。
 */
export const SCALES: readonly Scale[] = [1, 2, 4] as const;
/** 红线的机器可读版本 —— 调价后照它自查一遍就能发现倒挂 */
export const INDIVIDUAL_PER_CREDIT_FLOOR = 0.00634;

/**
 * 容量倍数的批量折扣,按周期分开写 —— 让 chip(= 1 - display/reference)显示的数字符合营销口径。
 *
 * Monthly bulk = chip 直接显示数字(2× = 33% off,4× = 40% off)
 * Yearly bulk = chip 显示「叠加 yearly 30% 之后的总折扣」
