...
export default function MarketingAgentMissions() {
  ...
  const pick = (mission: Mission) => {
    setDraft(mission.prompt);
    setAttached(mission.attachments);
      ...
      <div className="flex">
        <aside className="hidden w-[150px] shrink-0 flex-col gap-1 border-r border-[#ececf1] bg-white px-3 py-4 lg:flex">
          <div className="mb-3 flex items-center gap-2 px-2">
            ...
            <span className="font-[family-name:var(--font-display)] font-extrabold tracking-tight">Buzz</span>
          </div>
        ...
        <main className="min-w-0 flex-1">
          <header className="flex items-center justify-end gap-3 px-6 py-3">
            <span className="flex items-center gap-1.5 rounded-full bg-[#fff3ec] px-3 py-1.5 text-xs font-bold text-[#ff5e1a]">63,016 credits</span>
            <button className={`rounded-full ${ctaGrad} px-4 py-1.5 text-sm font-bold text-white shadow-[0_8px_20px_rgba(255,82,85,0.28)]`}>Upgrade</button>
          ...
          <div className="px-6">
            <h1 className="mt-6 text-center font-[family-name:var(--font-display)] text-[clamp(28px,4vw,40px)] font-extrabold leading-[1.12] tracking-tight">
              ...
              <br className="hidden sm:block" /> in minutes with <span className={gradText}>Buzz Agent</span>
            </h1>
              ...
              <div className="flex h-[178px] flex-col rounded-[22px] border border-[#ececf1] bg-white p-3.5 shadow-[0_4px_16px_rgba(26,26,46,0.06)] transition focus-within:border-[#ff5e1a] focus-within:ring-2 focus-within:ring-[#ff5e1a]/20">
                  ...
                  <div className="mb-1.5 flex flex-wrap items-center gap-2 px-1">
                    {attached.map((asset) => (
                      <div key={asset.label} className="relative">
                        {asset.type === "video" ? (
                          <video src={asset.url} muted playsInline className="size-11 rounded-lg object-cover ring-1 ring-[#ececf1]" />
                        ) : (
                          <img src={asset.url} alt={asset.label} className="size-11 rounded-lg object-cover ring-1 ring-[#ececf1]" />
                        )}
                        <button
                          onClick={() => setAttached((previous) => previous?.filter((item) => item.label !== asset.label))}
                ...
                <div className="flex items-center justify-between gap-2 px-1 pt-2">
                  <div className="flex items-center gap-2 text-[#6a6b7b]">
                    <span className="grid size-8 place-items-center rounded-lg border border-[#ececf1]"><Plus className="size-4" /></span>
                    <span className="flex items-center gap-1.5 rounded-lg border border-[#ececf1] px-2.5 py-1.5 text-xs font-semibold"><Sparkles className="size-3.5 text-[#ff5e1a]" /> Marketing Agent <ChevronDown className="size-3.5" /></span>
                    <span className="hidden items-center gap-1.5 rounded-lg border border-[#ececf1] px-2.5 py-1.5 text-xs font-semibold sm:flex"><SlidersHorizontal className="size-3.5" /> Auto</span>
                    <History className="hidden size-4 text-[#9a9bb0] sm:block" />
                  </div>
                  <button className={`grid size-9 place-items-center rounded-xl ${ctaGrad} text-white shadow-[0_8px_20px_rgba(255,82,85,0.3)] transition hover:brightness-105`} aria-label="Generate"><ArrowUp className="size-4" /></button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white px-6 py-14 pb-24">
            <section>
              <h3 className="mb-3 text-sm font-bold text-[#1a1a2e]">Campaign Creation</h3>
              <div className="flex gap-4 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {CAMPAIGN_TEMPLATES.map((template) => (
                  <div key={template} className="w-[180px] shrink-0">
                    <div className="aspect-[4/3] rounded-xl bg-gradient-to-br from-[#f1f0f4] to-[#e7e6ec]" />
                    <p className="mt-2 truncate text-xs font-medium text-[#6a6b7b]">{template}</p>
                  </div>
                ))}
              </div>
            </section>
